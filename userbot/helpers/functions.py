...
#gban
async def admin_groups(wolf):
    wolfgroups = []
    async for dialog in wolf.client.iter_dialogs():
        entity = dialog.entity  
        if isinstance(entity, Channel):
            if entity.megagroup:
                if entity.creator or entity.admin_rights:
                   wolfgroups.append(entity.id)
    return wolfgroups

#for getmusic
async def wolfmusic(wolf , QUALITY):
  search = wolf
...
async def waifutxt(text, chat_id ,reply_to_id , bot, borg):
    ...
    wolf = await sticcers[0].click( "me" ,
                            hide_via=True)
    if wolf:
        await borg.send_file(int(chat_id) , wolf , reply_to = reply_to_id ) 
        await wolf.delete()
