...
async def _(event):
    ...
    if reply_message.sender.bot:
       ...
       return
    wolf = await event.edit("recognizeing this media")
    async with event.client.conversation(chat) as conv:
          ...
          if response.text.startswith("See next message."):
              response = conv.wait_event(events.NewMessage(incoming=True,from_users=461083923))
              response = await response
              wolf = response.message.message
              await event.edit(wolf)
...
async def _(event):
    ...
    if reply_message.sender.bot:
       ...
       return
    wolf = await event.reply("recognizeing this media")
    async with event.client.conversation(chat) as conv:
          try:     
              response = conv.wait_event(events.NewMessage(incoming=True,from_users=461083923))
              await event.client.forward_messages(chat, reply_message)
