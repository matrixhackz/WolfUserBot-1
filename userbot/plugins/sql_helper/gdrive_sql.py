"""
credits to @mrconfused and @sammy1709
"""
...
class Gdrive(BASE):
    __tablename__ = "wolfgdrive"
    wolf= Column(String(50), primary_key=True)
...
def rmparent_id(folder_id):
    note = SESSION.query(Gdrive).filter(Gdrive.wolf == folder_id)
    if note:
        note.delete()
        SESSION.commit()
